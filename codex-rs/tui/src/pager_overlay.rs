...
pub(crate) enum Overlay {
    ...
    BackgroundTerminalDetail(BackgroundTerminalDetailOverlay),
}
...
impl Overlay {
    ...
    pub(crate) fn new_background_terminal_detail(
        process_key: String,
        lines: Vec<Line<'static>>,
    ) -> Self {
        Self::BackgroundTerminalDetail(BackgroundTerminalDetailOverlay::new(process_key, lines))
    }
    ...
    pub(crate) fn handle_event(&mut self, tui: &mut tui::Tui, event: TuiEvent) -> Result<()> {
        match self {
            ...
            Overlay::BackgroundTerminalDetail(o) => o.handle_event(tui, event),
        }
    }
    ...
    pub(crate) fn is_done(&self) -> bool {
        match self {
            ...
            Overlay::BackgroundTerminalDetail(o) => o.is_done(),
        }
    }
}
...
pub(crate) struct BackgroundTerminalDetailOverlay {
    process_key: String,
    view: PagerView,
    is_done: bool,
}

impl BackgroundTerminalDetailOverlay {
    fn new(process_key: String, lines: Vec<Line<'static>>) -> Self {
        let paragraph = Paragraph::new(Text::from(lines)).wrap(Wrap { trim: false });
        Self {
            process_key,
            view: PagerView::new(
    ...
    pub(crate) fn process_key(&self) -> &str {
        &self.process_key
    }

    pub(crate) fn sync_lines(&mut self, lines: Vec<Line<'static>>) {
        let follow_bottom = self.view.is_scrolled_to_bottom();
        let paragraph = Paragraph::new(Text::from(lines)).wrap(Wrap { trim: false });
